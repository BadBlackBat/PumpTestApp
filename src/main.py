...
def compute_scale_factor():
    """Определяет коэффициент масштабирования ВСЕГО интерфейса целиком
    (шрифты, отступы, фиксированные размеры виджетов) по реальному
    физическому разрешению экрана - НЕ то же самое, что процент
    масштабирования Windows (Панель управления -> Экран): у ноутбука
    с экраном 1366x768 он вполне может быть выставлен как обычные 100%,
    хотя фактического места на экране физически меньше, чем на Full HD.

    Программа спроектирована с расчётом на Full HD (1920x1080) - это
    ...
    (но никогда не увеличивается на больших - там и так всё нормально
    выглядит, а искусственное увеличение сделало бы интерфейс
    непропорционально крупным). Нижняя граница 0.7 - дальше текст
    рисковал бы стать нечитаемым, даже на совсем маленьких экранах.

    Работает через прямой системный вызов Windows (GetSystemMetrics), а
    не через QApplication.primaryScreen() - экран нужно узнать ДО
    создания QApplication, поскольку переменная окружения
    QT_SCALE_FACTOR (см. main()) должна быть установлена заранее: Qt
    читает её только один раз, при собственной инициализации. На
    других ОС (не Windows) возвращает 1.0 без изменений - масштабируем
    только на Windows, где эта проблема практически и встречается."""
    if sys.platform != "win32":
        return 1.0
    try:
...
def main():
    ...
    # Коэффициент масштабирования всего интерфейса по реальному
    # разрешению экрана (см. compute_scale_factor) - переменная
    # окружения ОБЯЗАТЕЛЬНО должна быть установлена ДО создания
    # QApplication, иначе Qt её просто не подхватит (читает только один
    # раз, при собственной инициализации)
