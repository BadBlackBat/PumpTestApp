...
def is_small_screen():
    """True на компактных экранах (1366x768 и подобных) - там UI_SCALE
    занижен относительно эталонных 1920x1080 (см. main.py:
    compute_ui_scale). Используется в отдельных особо "тесных" диалогах
    (редактирование протокола, добавление модификации), чтобы точечно
    ужимать именно их внутренние отступы - в отличие от scaled(), это
    не общий множитель всего интерфейса, а явная развилка "маленький
    экран / нет" для мест, где на большом экране, наоборот, нужно
    больше воздуха, а не просто "не меньше базового размера"."""
    return UI_SCALE < 1.0


def dialog_extra_spacing(base_pixels):
    """Для НЕСКОЛЬКИХ особо тесных диалогов (редактирование протокола,
    добавление модификации) - на большом экране (не тесном) немного
    увеличивает базовый отступ, чтобы окно не выглядело сжатым на
    просторном экране (adjustSize() в _GlowDialog._lock_size() иначе
    берёт минимально необходимый размер и никогда сам не "растягивает"
    контент). На маленьком экране отступ не трогаем - там и так важнее
    компактность, см. is_small_screen().

    ВАЖНО: прибавка ФИКСИРОВАННАЯ (не множитель) - у этих диалогов
    подряд идёт несколько разных отступов (между строками полей, между
    блоками таблиц, перед примечанием и т.п.), и множитель на каждом из
    них суммарно "накручивал" итоговую высоту окна намного сильнее, чем
    ожидалось (было замечено как непропорционально большие пустые
    промежутки между абзацами)."""
    if is_small_screen():
        return scaled(base_pixels)
    return scaled(base_pixels + 6)


def set_dynamic_style(widget, css):
    """Задаёт виджету стиль, который может ещё не раз поменяться в
    рантайме (например, оранжевая подсветка изменённого значения в
    EditPumpDialog - см. _on_seal_changed/_on_pressure_changed) - в
    отличие от прямого widget.setStyleSheet(css), сразу же обновляет и
    закэшированный retheme_widget_tree()'ом "_original_stylesheet".

    Без этого следующий вызов retheme_widget_tree() на этом же диалоге
    (он происходит заново при каждой смене модификации, см.
    EditPumpDialog.on_modification_changed) находит СТАРЫЙ кэш - стиль
    виджета ДО подсветки - и молча перезаписывает им текущий, реально
    видимый пользователем стиль. Внешне это выглядело как ничем не
    объяснимое, будто периодическое, возвращение фона/цвета отдельных
    полей к исходному виду - виджет не "ломался", просто ретемизация
    откатывала его к устаревшему снимку. Все места в коде, которые
    меняют стиль виджета ПОСЛЕ его создания (а не один раз при
    построении), должны использовать эту функцию, а не setStyleSheet
    напрямую."""
    widget.setStyleSheet(css)
    widget.setProperty("_original_stylesheet", css)
