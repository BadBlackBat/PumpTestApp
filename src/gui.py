...
class MainWindow(QMainWindow):
    ...
    def on_upload_requested(self):
        ...
        # Предупреждаем ТОЛЬКО если реально есть несохранённые изменения -
        # если уже всё синхронизировано, действие и так ничего не
        # изменит, предупреждать здесь не о чем (лишнее "ОК ради ОК"
        # быстро приучило бы нажимать не глядя)
        if db_settings.is_network_mode_active() and not db_sync.is_synced():
            if not GlowMessageDialog.confirm(
                self, "Выгрузка в сеть",
                "Вы собираетесь выгрузить свои локальные изменения в "
                "общую сетевую базу - это затронет всех, кто использует "
                "программу.\n\nПродолжить?"
            ):
                return
    ...
    def on_force_pull_requested(self):
        ...
        кнопке "N -> L" в панели фильтров. Предупреждает и ждёт
        подтверждения в любом случае (не отбирает у пользователя право
        отменить действие) - формулировка серьёзнее именно тогда, когда
        есть реальный риск потери несохранённых локальных изменений."""
        ...
        if not db_sync.is_synced():
            ...
            warning_text = (
                "Локальная база данных будет заменена текущей сетевой "
                "версией.\n\nПродолжить?"
            )
        if not GlowMessageDialog.confirm(self, "Копирование сетевой базы", warning_text):
            return
    ...
    def closeEvent(self, event):
        ...
        # Останавливаем таймер фоновой проверки сети - без этого он мог
        # бы запустить ЕЩЁ один _RemoteCheckWorker уже после того, как
        # мы решили закрываться. И ждём (с разумным таймаутом, а не
        # бесконечно - сеть может зависнуть) уже запущенный поток, если
        # он в этот момент как раз выполняется - иначе QThread уничтожался
        # бы прямо во время работы run() (обращение к сети), что и давало
        # в консоли "QThread: Destroyed while thread is still running" и
        # теоретически могло привести к падению, а не только к предупреждению.
        self._remote_check_timer.stop()
        if self._remote_check_worker is not None and self._remote_check_worker.isRunning():
            self._remote_check_worker.wait(3000)
