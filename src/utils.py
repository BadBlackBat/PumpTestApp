...
# Кириллические буквы, визуально неотличимые от латинских - типичная
# путаница при ручном вводе номера заказа (например, "К56" кириллицей и
# "K56" латиницей выглядят абсолютно одинаково, но для базы данных это
# разные строки, и насосы "разъезжаются" по разным заказам, хотя на
# самом деле это один и тот же заказ)
...
def normalize_order_number(value):
    """Приводит номер заказа к единому виду для хранения/сравнения:
    - заменяет кириллические буквы-омоглифы на латинские аналоги
      (визуально неотличимы, но для базы данных - разные символы);
    - убирает пробелы по краям;
    - приводит к верхнему регистру (чтобы "к56" и "К56" тоже не
      считались разными заказами).
    Использовать везде, где номер заказа вводится вручную или
    импортируется - иначе один и тот же заказ может "разъехаться" на
...
def format_order_number(value):
    """Приводит номер заказа к строке без хвостового float-артефакта
    (например, "5644.0" из Excel/pandas -> "5644"). Убирает ТОЛЬКО
    буквальное ".0" СТРОГО В КОНЦЕ строки (через re.sub с якорем $), а
    не любое вхождение ".0" где бы оно ни было - раньше здесь была
    дополнительная строка s.replace('.0', ''), которая срабатывала на
    ЛЮБОМ вхождении подстроки ".0" внутри номера, портя настоящие
    номера заказов вроде "5.087" (превращая их в "587" - точка и ноль
    сразу после неё вырезались, даже когда это не имело отношения к
    float-артефакту). Тот же точный подход используется и в
    database.add_order() - единообразно по всему проекту."""
    if value is None:
        return '—'
