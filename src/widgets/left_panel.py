...
def _make_people_icon(color, size=14, count=1):
    ...
    мониторов" (count>1, два раздельно, не внахлёст) - программно через
    QPainter, без отдельных файлов ресурсов. Используется для индикатора
    количества активных пользователей сетевой базы (см.
    db_sync.get_active_user_count).
    ...
    # Для "нескольких" - холст шире (иначе два монитора при таком мелком
    # размере попросту некуда было раздвинуть без наложения друг на друга)
    width = size if count <= 1 else int(size * 1.6)
    pixmap = QPixmap(width, size + 2)
    ...
    if count <= 1:
        ...
        # Раздельно, с явным зазором между ними - раньше центры стояли
        # слишком близко (0.38/0.66 от узкой ширины size), и мониторы
        # заметно перекрывали друг друга, сливаясь в одно пятно
        draw_monitor(width * 0.30, 0.72)
        draw_monitor(width * 0.74, 0.72)
